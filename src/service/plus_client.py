        ...
        try:
            if pyigtl:  # si la bibliothèque pyigtl est disponible → mode réel (connexion à PlusServer)
                ...
                try:
                    ...
                    arr = np.zeros((256, 256), dtype=np.uint8)  # si échec, crée une image vide 256x256 (sécurité)
                ...
                try:
                    ...
                    try:
                        ...
                        increment_drops("rx.drop_total", 1, emit=True)
                    except Exception:
                        pass
                ...
                h, w = 256, 256  # dimensions simulées de l’image
                arr = (np.random.rand(h, w) * 255).astype(np.uint8)  # crée une image aléatoire en niveaux de gris
                ...
                rf = RawFrame(image=arr, meta=meta)  # encapsule les données dans un RawFrame simulé
                try:
